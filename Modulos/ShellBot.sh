...
if ! awk 'BEGIN { exit ARGV[1] < 4.3 }' ${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}; then
	echo "${BASH_SOURCE:-${0##*/}}: erro: requer o interpretador de คำสั่งs 'bash 4.3' ou superior." 1>&2
	exit 1
fi
...
MessageError()
{
	...
	# TG - Erro externo retornado โดย core do telegram.
	# API - Erro interno gerado โดย API do ShellBot.
	case $1 in
		TG)
