...
archAffix(){
    case "$(uname -m)" in
      ...
      's390x')
        ...
        ;;
        *)
        echo "ผิดพลาด: ไม่รองรับสถาปัตยกรรมนี้"
        exit 1
        ;;
    esac
...
downloadV2Ray(){
    ...
    colorEcho ${BLUE} "กำลังดาวน์โหลด $KEY: ${DOWNLOAD_LINK}"
    curl ${PROXY} -L -H "Cache-Control: no-cache" -o ${ZIPFILE} ${DOWNLOAD_LINK}
    if [ $? != 0 ];then
        colorEcho ${RED} "ดาวน์โหลดล้มเหลว! กรุณาตรวจสอบเครือข่ายแล้วลองใหม่"
        return 3
    fi
    return 0
}
...
installSoftware(){
    ...
    getPMT
    if [[ $? -eq 1 ]]; then
        colorEcho ${RED} "ตัวจัดการแพ็กเกจของระบบไม่ใช่ APT หรือ YUM กรุณาติดตั้ง ${COMPONENT} ด้วยตนเอง"
        return 1
    fi
    if [[ $SOFTWARE_UPDATED -eq 0 ]]; then
        colorEcho ${BLUE} "กำลังอัปเดตรายการแพ็กเกจ"
        $CMD_UPDATE
        SOFTWARE_UPDATED=1
    fi

    colorEcho ${BLUE} "กำลังติดตั้ง ${COMPONENT}"
    $CMD_INSTALL $COMPONENT
    if [[ $? -ne 0 ]]; then
        colorEcho ${RED} "ติดตั้ง ${COMPONENT} ไม่สำเร็จ กรุณาติดตั้งด้วยตนเอง"
        return 1
    fi
    return 0
}
...
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        if [[ $? -ne 0 ]] || [[ $NEW_VER == "" ]]; then
            colorEcho ${RED} "ดึงข้อมูลเวอร์ชันใหม่ไม่สำเร็จ กรุณาตรวจสอบเครือข่ายแล้วลองใหม่"
            return 3
...
stopV2ray(){
    colorEcho ${BLUE} "กำลังปิดบริการ $KEY"
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${YELLOW} "ปิดบริการ $KEY ไม่สำเร็จ"
        return 2
    fi
    return 0
}
...
startV2ray(){
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${YELLOW} "เปิดบริการ $KEY ไม่สำเร็จ"
        return 2
    fi
    return 0
}
...
installV2Ray(){
    ...
    if [[ $KEY == "Xray" ]];then
        ...
        chmod +x /usr/bin/$KEY_LOWER/$KEY_LOWER || {
            colorEcho ${RED} "คัดลอกไฟล์ไบนารีและทรัพยากรของ $KEY ไม่สำเร็จ"
            return 1
        }
    else
...
EOF
            ...
            /etc/$KEY_LOWER/config.json || {
                colorEcho ${YELLOW} "สร้างไฟล์คอนฟิก $KEY ไม่สำเร็จ กรุณาสร้างด้วยตนเอง"
                return 1
            }
        fi
...
./go.sh [-h] [-c] [--remove] [-p proxy] [-f] [--version vx.y.z] [-l file] [-x]
  -h, --help            แสดงคำแนะนำ
  -p, --proxy           ดาวน์โหลดผ่านพร็อกซี เช่น -p socks5://127.0.0.1:1080 หรือ -p http://127.0.0.1:3128
  -f, --force           บังคับติดตั้ง
      --version         ติดตั้งเวอร์ชันที่ระบุ เช่น --version v3.15
  -l, --local           ติดตั้งจากไฟล์ในเครื่อง
      --remove          ลบ V2Ray/Xray ที่ติดตั้งไว้
  -x, --xray            โหมด Xray
  -c, --check           ตรวจสอบเวอร์ชันใหม่
EOF
}
...
remove(){
    if [[ -n "${SYSTEMCTL_CMD}" ]] && [[ -f "/etc/systemd/system/$KEY_LOWER.service" ]];then
        ...
        if [[ $? -ne 0 ]]; then
            colorEcho ${RED} "ลบ $KEY ไม่สำเร็จ"
            return 0
        else
            colorEcho ${GREEN} "ลบ $KEY สำเร็จ"
            colorEcho ${BLUE} "หากจำเป็น กรุณาลบไฟล์คอนฟิกและไฟล์ log ด้วยตนเอง"
            return 0
        fi
        ...
        colorEcho ${YELLOW} "ไม่พบ $KEY"
        return 0
    fi
}

checkUpdate(){
    echo "กำลังตรวจสอบเวอร์ชันใหม่..."
    VERSION=""
    getVersion
    RETVAL="$?"
    if [[ $RETVAL -eq 1 ]]; then
        colorEcho ${BLUE} "พบ $KEY เวอร์ชันใหม่ ${NEW_VER} (เวอร์ชันปัจจุบัน: $CUR_VER)"
    elif [[ $RETVAL -eq 0 ]]; then
        colorEcho ${BLUE} "ไม่มีเวอร์ชันใหม่ เวอร์ชันปัจจุบันคือ ${NEW_VER}"
    elif [[ $RETVAL -eq 2 ]]; then
        colorEcho ${YELLOW} "ยังไม่ได้ติดตั้ง $KEY"
        colorEcho ${BLUE} "เวอร์ชันล่าสุดของ $KEY คือ ${NEW_VER}"
    fi
    return 0
}
...
main(){
    ...
    if [[ $LOCAL_INSTALL -eq 1 ]]; then
        colorEcho ${YELLOW} "กำลังติดตั้ง $KEY จากไฟล์ในเครื่อง กรุณาตรวจสอบว่าไฟล์เป็นแพ็กเกจ $KEY ที่ถูกต้อง (ระบบตรวจสอบให้ไม่ได้)"
        NEW_VER=local
        rm -rf /tmp/$KEY_LOWER
        ZIPFILE="$LOCAL"
    else
        ...
        if [[ $RETVAL == 0 ]] && [[ "$FORCE" != "1" ]]; then
            colorEcho ${BLUE} "เวอร์ชันล่าสุด ${CUR_VER} ติดตั้งอยู่แล้ว"
            if [ -n "${ERRORR_IF_UPTODATE}" ]; then
              return 10
            fi
            return
        elif [[ $RETVAL == 3 ]]; then
            ...
            colorEcho ${BLUE} "กำลังติดตั้ง $KEY ${NEW_VER} บน ${ARCH}"
    ...
    if [ -n "${EXTRACT_ONLY}" ]; then
        colorEcho ${BLUE} "กำลังแตกแพ็กเกจ $KEY ไปยัง ${VSRC_ROOT}"

        if unzip -o "${ZIPFILE}" -d ${VSRC_ROOT}; then
            colorEcho ${GREEN} "แตก $KEY ไปยัง ${VSRC_ROOT%/}${ZIPROOT:+/${ZIPROOT%/}} แล้ว กำลังออกจากโปรแกรม..."
            return 0
        else
            colorEcho ${RED} "แตกแพ็กเกจ $KEY ไม่สำเร็จ"
            return 2
        fi
    fi
    ...
    if [[ ${V2RAY_RUNNING} -eq 1 ]];then
        colorEcho ${BLUE} "กำลังรีสตาร์ตบริการ $KEY"
        startV2ray
    fi
    colorEcho ${GREEN} "ติดตั้ง $KEY ${NEW_VER} เรียบร้อยแล้ว"
    rm -rf /tmp/$KEY_LOWER
    return 0
}
