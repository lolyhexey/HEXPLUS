...
while [[ $# > 0 ]];do
    ...
    case $KEY in
        --nopip)
        NO_PIP=1
        colorEcho $BLUE "ติดตั้งเฉพาะ python3..\n"
        ;;
        --latest)
        LATEST=1
        ;;
        -v|--version)
        INSTALL_VERSION="$2"
        echo -e "เตรียมติดตั้ง python $(colorEcho ${BLUE} $INSTALL_VERSION)..\n"
        shift
        ;;
        *)
...
if [[ $LATEST == 1 || $INSTALL_VERSION ]];then
    [[ $CONFIG_PARAM ]] && echo "คำสั่งคอมไพล์ python3: `colorEcho $BLUE ./configure $CONFIG_PARAM`"
fi
#############################

checkSys() {
    # check root user
    [ $(id -u) != "0" ] && { colorEcho ${RED} "ผิดพลาด: ต้องเป็นผู้ใช้ root จึงจะรันสคริปต์นี้ได้"; exit 1; }
    ...
    if [[ `command -v apt-get` ]];then
        ...
        colorEcho $RED "ไม่รองรับระบบปฏิบัติการนี้!"
        exit 1
    fi
...
downloadPackage(){
    ...
    do
        if [[ ! -e $PYTHON_PACKAGE ]];then
            ...
            if [[ $? != 0 ]];then
                colorEcho ${RED} "ดาวน์โหลด python เวอร์ชัน $PYTHON_PACKAGE ไม่สำเร็จ!"
                exit 1
            fi
        fi
